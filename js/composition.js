function initDraggableComposition() {
    ...
    function createDraggableImage() {
        ...
        draggableContainer.style.transition = 'transform 0.1s ease-out';
        draggableContainer.style.touchAction = 'none'; // 브라우저 기본 터치 동작 비활성화
    ...
    function setDraggable(element) {
        // 제스처 이벤트 리스너
        element.addEventListener('gesturestart', handleGestureStart, false);
        element.addEventListener('gesturechange', handleGestureChange, false);
        element.addEventListener('gestureend', handleGestureEnd, false);
    ...
    function handleGestureChange(e) {
        e.preventDefault();
        if (selectedElement && isZooming) {
            const newScale = initialGestureScale * e.scale;
    ...
    function handleTouchMove(e) {
        if (e.touches.length === 2 && isZooming && selectedElement) {
            e.preventDefault();
            const touch1 = e.touches[0];
            const touch2 = e.touches[1];
            const currentTouchDistance = getTouchDistance(touch1, touch2);
            const scaleDiff = currentTouchDistance / initialTouchDistance;
            const newScale = initialGestureScale * scaleDiff;
            currentScale = Math.min(Math.max(newScale, MIN_SCALE), MAX_SCALE);
            selectedElement.style.transform = `scale(${currentScale})`;
    ...
    function dragStart(e) {
        if (!isZooming) {
            ...
            if (selectedElement) {
                isDragging = true;
                initialX = (e.clientX || e.pageX) - parseFloat(selectedElement.style.left || 0);
                initialY = (e.clientY || e.pageY) - parseFloat(selectedElement.style.top || 0);
    ...
    function drag(e) {
        if (isDragging && selectedElement && !isZooming) {
            e.preventDefault();
